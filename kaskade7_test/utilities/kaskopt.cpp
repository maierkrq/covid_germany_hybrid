...
namespace Kaskade
{
  ...
  template Options::Options(char const* option, size_t& parameter, size_t const& defaultValue, char const* comment);
  template Options::Options(char const* option, double& parameter, double const& defaultValue, char const* comment);
  template Options::Options(char const* option, float & parameter, float  const& defaultValue, char const* comment);
  ...
  template Options& Options::operator()(char const* option, size_t& parameter, size_t const& defaultValue, char const* comment);
  template Options& Options::operator()(char const* option, double& parameter, double const& defaultValue, char const* comment);
  template Options& Options::operator()(char const* option, float & parameter, float  const& defaultValue, char const* comment);
...
std::unique_ptr<ptree> getKaskadeOptions(int argc, char *argv[], int verbosity, bool dump)
{
  ...
  string optionFile("default.json"), empty;
  ...
  try
  {
    ...
    else
    {
      ...
      {
        std::cerr << "Could not read option file " << optionFile << "\n";
      }
      if (verbosity>0)
