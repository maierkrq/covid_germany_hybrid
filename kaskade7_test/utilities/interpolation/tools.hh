...
namespace Kaskade
{
  ...
  bool adjustDampingFactor(Grid& grid, Deformation const& deformation, Vector const& initialVolume, DeformationPolicy const& deformationPolicy, DampingPolicy& dampingPolicy)
  {
    ...
    // iterate over cells
    std::for_each(grid.leafGridView().template begin<0>(), grid.leafGridView().template end<0>(), [&](typename Grid::LeafGridView::template Codim<0>::Iterator::Entity const& entity)
        {
      LocalGeometryInCoarseGridAncestor<Grid> lgcga(grid, Cell(entity));
      ...
      {
        typename Dune::template UGGridFamily<dim, dim>::Traits::template Codim<dim>::Entity e(entity.template subEntity<dim>(i));
        ...
        //}
        grid.setPosition(e,w);
      }
        });

    ctype minvol = 1000;
    ctype minrel = 1000;
    auto cend = grid.leafGridView().template end<0>();
    for(auto ci = grid.leafGridView().template begin<0>(); ci!=cend; ++ci)
    {
      if(ci->geometry().volume()/initialVolume[grid.leafGridView().indexSet().index(*ci)] < minrel) minrel = ci->geometry().volume()/initialVolume[grid.leafGridView().indexSet().index(*ci)];
      if(ci->geometry().volume() < minvol) minvol = ci->geometry().volume();
      if(deformationPolicy(*ci, 0.3*initialVolume[grid.leafGridView().indexSet().index(*ci)])) return false;
    }
      ...
      DampingPolicy<typename Grid::ctype> dampingPolicy = DampingPolicy<typename Grid::ctype>())
  {
    ...
    std::vector<typename Grid::ctype> initialVolume(grid.leafGridView().indexSet().size(0),0);
    std::for_each(grid.leafGridView().template begin<0>(), grid.leafGridView().template end<0>(), [&initialVolume,&grid](typename CellIterator::Entity const& entity)
        {
      for(int i=0; i<entity.geometry().corners(); ++i) initialVolume[grid.leafGridView().indexSet().index(entity)] = entity.geometry().volume();
        });
