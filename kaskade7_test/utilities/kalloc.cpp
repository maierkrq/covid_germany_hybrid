...
namespace Kaskade 
{
  namespace
  {
    ...
    void* sysAlloc(size_t n, int node)
    {
      ...
      if (p == nullptr)
        throw DetailedException("Memory allocation of " + std::to_string(n) + " bytes on NUMA node " + std::to_string(node) + "failed.\n",
                                __FILE__,__LINE__);
      return p;
      #else
      return std::malloc(n);
  ...
  {
     std::lock_guard<std::mutex> lock(kalloc.mutex);
  ...
  std::pair<void*,size_t> Kalloc::findFreePage(int b)
  {
    ...
    for (int c=b; c<freeChunks.size(); ++c)
  ...
  {
    assert(0<=b && b<freeChunks.size() && !freeChunks[b].empty());
  ...
  void Kalloc::coalesce()
  {
    ...
    {
      int j = i;
      while (j+1<ranges.size() && ranges[j].first+ranges[j].second==ranges[j+1].first)
        ++j;
  ...
  void Kalloc::merge(Iter first, Iter last)
  {
    ...
    for (Iter i=first; i!=last; ++i)
    {
      ...
      freeChunks[b].erase(it); // don't need to check for not found - we know it must be there
      n += bs;
    }
    std::cerr << "\n";
