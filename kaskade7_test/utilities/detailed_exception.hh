...
namespace Kaskade{
  ...
  /**
   * \ingroup exceptions
   * \brief To be raised if an iterative solver fails.
   */
  class IterativeSolverException: public DetailedException
  {
  public:
    IterativeSolverException(std::string const& message, std::string const& file, int const line):
      DetailedException(std::string("Iterative solver failed.\n")+message,file,line)
    {
    }
  };
  ...
  /**
   * \ingroup exceptions
   * \brief To be raised if an iterative solver detects the problem to be infeasible (has no valid solution).
   */
  class InfeasibleProblemException: public IterativeSolverException
  {
  public:
    InfeasibleProblemException(std::string const& message, std::string const& file, int const line):
      IterativeSolverException(std::string("Problem has no solution (infeasible).\n")+message,file,line)
    {
    }
  };
  ...
  /**
   * \ingroup exceptions
   * \brief To be raised if an iterative solver detects the optimization problem to be unbounded (has no valid solution).
   */
  class UnboundedProblemException: public IterativeSolverException
  {
  public:
    UnboundedProblemException(std::string const& message, std::string const& file, int const line):
      IterativeSolverException(std::string("Optimization problem has no solution (unbounded).\n")+message,file,line)
    {
    }
  };
  ...
  /**
   * \ingroup exceptions
   * \brief To be raised if a linesearch fails.
   */
  class LinesearchException: public IterativeSolverException
  {
  public:
    LinesearchException(std::string const& message, std::string const& file, int const line):
      IterativeSolverException(std::string("Linesearch failed.\n")+message,file,line)
    {
    }
  };
  ...
  /**
   * \ingroup exceptions
   * \brief An exception class for grid related errors.
   */
  class GridException: public DetailedException 
  {
  public:
    GridException(std::string const& message, std::string const& file, int const line) :
      DetailedException("Grid error.\n"+message,file,line) 
    {
    }
  };
