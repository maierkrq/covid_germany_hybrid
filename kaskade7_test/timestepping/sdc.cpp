...
//  Hotfix for clang++ -- L.Weimann
  ...
  void luIntegrationMatrix(SDCTimeGrid const& grid, SDCTimeGrid::RealMatrix& Shat)
  {
    ...
    //       [Update 2018-05] Pivoting appears to result in P=I in general. So probably we could use LAPACK.
    for (int k=0; k<n-1; ++k)
    {
      assert(A[k][k] != 0);
      for (int i=k+1; i<n; ++i)
...
//  Hotfix for clang++ -- L.Weimann
  ...
  namespace {
    ...
    // Computes the n Gauss nodes on ]-1,1[, and stores them together with first point -1 in t. n>=1
    void computeGaussNodes(int n, Dune::DynamicVector<double>& t)
    {
      assert(n >= 1);
      ...
      std::vector<double> diag(n,0.0), subdiag(n-1); 
      for (int i=1; i<n; ++i)
  ...
  void LobattoTimeGrid::refine(DynamicMatrix<double>& p)
  {
  ...
  void RadauTimeGrid::refine(DynamicMatrix<double>& p)
  {
    Dune::DynamicVector<double> newPts(pts.size()+1);
  ...
  GaussTimeGrid::GaussTimeGrid(int n, double a, double b_): b(b_)
  {
    assert(n>=1);
    computeGaussNodes(n,pts);
  ...
  void GaussTimeGrid::refine(DynamicMatrix<double>& p)
  {
    Dune::DynamicVector<double> newPts(pts.size()+1);
    computeGaussNodes(pts.size(),newPts);
    // rescale from [-1,1] to [a,b]
    double const a = pts[0];
    for (int i=0; i<newPts.size(); ++i)
      newPts[i] = a + (newPts[i]+1)*(b-a)/2;
    computeLagrangeEvaluationMatrix(pts,newPts,p);
    pts = newPts;
  ...
  SDCTimeGrid::RealMatrix GaussTimeGrid::interpolate(SDCTimeGrid::RealVector const& x) const 
  {
    SDCTimeGrid::RealMatrix p;
    computeLagrangeEvaluationMatrix(points(),x,p);
    return p;
  ...
  void GaussTimeGrid::computeMatrices()
  {
    int n = pts.size()-1;
...
int main(void) 
{
  ...
  std::cerr << "Testing Lobatto time grid\n";
  {
    ...
    DynamicMatrix<double> const& A = grid.integrationMatrix();
    double one = 0;
    for (int i=0; i<n; ++i)
      for (int j=0; j<=n; ++j)
        one += A[i][j];
    ...
    DynamicMatrix<double>  P(n+2,n+1);
    grid.refine(P);
    for (int i=0; i<=n+1; ++i)
    {
      double one = 0;
