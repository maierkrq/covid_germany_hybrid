...
namespace Kaskade {
   ...
   * time grid points (including the end points). Polynomial interpolation, quadrature, and differentiation 
   * are provided using (a subset of) the grid points as nodes.
   * 
   * Conceptually, the collocation polynomials are defined by Lagrange (or Hermite) interpolation of their values
   * at the initial point \f$ t_0 \f$ and \f$ n \f$ collocation points (nodes) \f$ t_1,\dots,t_n \f$. Grid points
   * may coincide, in which case Hermite interpolation is used. Collocation conditions for \f$ \dot u = f(u) \f$ are to
   * be satisfied only at the nodes, i.e. \f$ \dot u(t_i) = f(u(t_i)), \quad i=1,\dots,n\f$.
   */
  class SDCTimeGrid
  {
  public:
     ...
     * 
     * This computes a matrix such that derivatives given by values at \f$ t_1,\dots,t_n\f$ can be easily integrated.
     * 
     * Interpolation is based on the nodes \f$t_1\dots,t_n\f$, omitting the starting point.
     * The Lagrangian interpolation functions \f$ L_k \f$ are defined by \f$ L_k(t_i) = \delta_{ik},\quad i=1,\dots,n \f$.
     * The matrix \f$ S \in \mathbb{R}^{n\times n+1}\f$ contains the values 
     * \f[ S_{ik} = \int_{\tau=t_i}^{t_{i+1}} L_k(\tau) \, d\tau \f]
     * (the leading column being zero).
     * This way, if \f$ u \f$ is defined in terms of its function values \f$ v_i = u(t_i),\quad i=1,\dots,n \f$, the 
     ...
     *
     * This computes a matrix such that polynomials given by values at \f$ t_0, \dots,t_n\f$ can easily be differentiated.
     * 
     * The Lagrangian interpolation functions \f$ L_k \f$ are defined by \f$ L_k(t_i) = \delta_{ik} , \quad i=0,\dots,n\f$.
     * The matrix \f$ D \in \mathbb{R}^{n+1\times n+1}\f$ contains the values 
     * \f[ D_{ik} = \dot L_k(t_i)  \f]
   ...
   * 
   * \param[out] Shat the approximate integration matrix, is resized as needed
   * 
   * \relates sdcIterationStep
   */
   ...
   * b.c. realized as quadratic penalty). Consider using GaussTimeGrid or LobattoTimeGrid in other cases.
   */
  ...
  /**
   * \ingroup timestepping
   * \brief spectral time grid for defect correction methods with Gauss points
   * 
   * Note that collocation with Gauss points is A-stable, and therefore SDC methods
   * on Gauss grids are best for oscillatory problems (e.g., hyperbolic problems). 
   * Consider using RadauTimeGrid in other cases.
   */
  class GaussTimeGrid: public SDCGridImplementationBase
  {
  public:
    /**
     * \brief constructs a Gauss grid with \f$ n+1 \f$ points on \f$ [a,b] \f$
     * 
     * This may throw LinearAlgebraException.
     */
   ...
   * vector \a u contains the approximate values of \f$ u \f$ at the time nodes \f$ t_i \f$ in a
   * Lagrangian FE basis.
   * Here, \a A and \a M are fixed matrices, such that locally we perform a method of lines
   * in this time step.
   * 
   ...
   * \param[in] Shat the triangular quadrature matrix approximation \f$ \hat S \f$ (see eulerIntegrationMatrix())
   ...
   * \param[in] rUi the right hand sides at the collocation times (including interval start and end point), size n+1
   ...
   * \param[in] u the current iterate (values of \f$ u \f$ at the collocation times including start and end point), size n+1
   * \param[out] du the approximate Newton correction \f$ \delta u \f$, size >=n+1
   */
...
// std::cerr << "|s| = " << snorm << "\n";      
  ...
  /**
   * \ingroup timestepping
   * \brief Encapsulates the state of an SDC fixed point iteration and provides stepping.
   * 
   * This solves the initial value problem \f$ \dot u = f(u) \f$ on the interval \f$ [t_0,T] \f$ by spectral deferred correction.
   * The equivalent Picard equation 
   * \f[ u(t) = u(a) + \int_{\tau=t_0}^t f(u(\tau)) \, d\tau \f]
   * is considered, and approximated by discretizing \f$ u\in S\times[t_0,T] \f$ by some polynomial \f$ P_{\le N}[S] \f$ and 
   * replacing the integral by quadrature on collocation points \f$ t_1, \dots, t_N \in \mathopen]t_0,T]\f$. 
   * Representing the polynomial \f$ u \f$ by its values \f$ u_i \f$ at \f$ t_i \f$, \f$ i=0,\dots, N\f$, we obtain
   * \f[ u_{i+1} = u_i + \sum_j S_{ij} f(u_j). \f]
   * Given some approximation \f$ \hat u \f$, the error \f$ \delta u = u - \hat u\f$ satisfies
   * \f[ \begin{aligned} \delta u_{i+1} &= u_i + \sum_j S_{ij} f(u_j) - \hat u_i + (\hat u_i - \hat u_{i+1}) \\
   *      &= \delta u_i + \sum_j S_{ij} f(\hat u_j + \delta u_j) + (\hat u_i - \hat u_{i+1}) \end{aligned}. \f]
   * Linearizing \f$ f \f$ around \f$ \hat u_j \f$ and splitting the sum leads to 
   * \f[ \delta u_{i+1} = \delta u_i + \sum_j S_{ij} f(\hat u_j) - (\hat u_{j+1} - \hat u_j) 
   *     + \sum_j S_{ij} f'(\hat u_j) \delta u_j, \f]
   * which can be simplified by replacing \f$ S \f$ by a lower triangular approximation \f$ \hat S \f$. This 
   * defines a correction \f$ \hat \delta u \f$ and consequently a fixed point iteration:
   * \f[ \begin{gathered}\hat \delta u_{i+1} := \hat\delta u_i + \sum_j S_{ij} f(\hat u_j) - (\hat u_{j+1} - \hat u_j) 
   *     + \sum_{j\le i+1} \hat S_{ij} f'(\hat u_j) \hat\delta u_j, \\
   *     \hat u \gets \hat u + \hat \delta u. \end{gathered} \f]
   * 
   * \tparam Vector type of vectors from some linear space. 
   */
  ...
  public:
    ...
    /**
     * \brief Sets the approximate quadrature matrix.
     * \param Shat lower triangular approximate quadrature matrix
     */
    ...
    /**
     * \brief Sets \f$ u \gets u + e^{-(t-t_0)d} (u_0-u) \f$
     */
