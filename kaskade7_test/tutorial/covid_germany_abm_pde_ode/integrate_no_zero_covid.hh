...
{ 
  ...
  std::vector<std::vector<double>> data_I(nr_PDE_states);
  std::vector<std::vector<std::vector<double>>> data_DF(nr_PDE_states);
  std::vector<std::vector<std::vector<double>>> data_DI(nr_PDE_states);
  for (int state_PDE_idx=0; state_PDE_idx<nr_PDE_states; state_PDE_idx++) { // iterate over all PDE domains
    data_F.at(state_PDE_idx).resize(nbr_points_PDE.at(state_PDE_idx) * (maxSteps+1),0.0);
    data_I.at(state_PDE_idx).resize(nbr_points_PDE.at(state_PDE_idx) * (maxSteps+1),0.0);
    data_DF.at(state_PDE_idx).resize(nbr_points_PDE.at(state_PDE_idx) * (maxSteps+1),std::vector<double>(1,0.0));
    data_DI.at(state_PDE_idx).resize(nbr_points_PDE.at(state_PDE_idx) * (maxSteps+1),std::vector<double>(1,0.0));
  }
  ...
  std::vector<std::vector<double>> I_PDE(nr_PDE_states,std::vector<double>(maxSteps+1, 0.0));
  ...
  std::vector<std::string> variables_infectious = {"i"};   
  ...
  if (verbosity > 0 || zero_covid || no_covid) { 
    ...
    for (int state_PDE_idx=0; state_PDE_idx<nr_PDE_states; state_PDE_idx++) { // iterate over all PDE domains
      ...
      #pragma omp critical(vtk_write)
      {
        ...
        writeVTK(x_PDE.at(state_PDE_idx),fn.str(),IoOptions().setPrecision(10),data_I.at(state_PDE_idx),data_DI.at(state_PDE_idx),variables_infectious,0,ones.at(state_PDE_idx),verbosity);
        // writeVTK(x_PDE.at(state_PDE_idx),fn.str(),IoOptions().setPrecision(10),data_n.at(state_PDE_idx),data_Dn.at(state_PDE_idx),variables_n,0,ones.at(state_PDE_idx),verbosity);
      }
      if (verbosity > 0) {
        ...
        for (auto const& cell: elements(x_PDE.at(state_PDE_idx).descriptions.gridView)) {
          ...
          for (int corner=0; corner<cell.subEntities(2); corner++) {
            ...
            I_PDE.at(state_PDE_idx)[0] += (area/3) * data_I.at(state_PDE_idx)[index];
          } // end for corner
        } // end for cell
    ...
    for (int state_PDE_idx=0; state_PDE_idx<nr_PDE_states; state_PDE_idx++) { // iterate over all PDE domains
      ...
      if ( verbosity>0 || zero_covid || no_covid) {
        ...
        #pragma omp critical(vtk_write)
        {
          ...
          writeVTK(x_PDE.at(state_PDE_idx),fm.str(),IoOptions().setPrecision(10),data_I.at(state_PDE_idx),data_DI.at(state_PDE_idx),variables_infectious,steps+1,ones.at(state_PDE_idx),verbosity); //density
