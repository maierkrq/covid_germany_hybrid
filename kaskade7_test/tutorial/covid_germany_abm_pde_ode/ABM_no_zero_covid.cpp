...
private:
    ...
    int dt_inv;
    bool zero_covid, no_covid;
    
public:
    ABModel(double population_scale_, bool with_restriction_, int dt_inv_, bool zero_covid_, bool no_covid_): population_scale(population_scale_), with_restriction(with_restriction_), dt_inv(dt_inv_), zero_covid(zero_covid_), no_covid(no_covid_) {
        ...
        for (int i = 0; i < dayData_size; i++) {
            ...
            if ((!event_of_last_agent) && (dt*step >= day_shift_edu_lockdown) && (category_id >= 7) && (category_id != 10)) { // category_id == 7, 8, 9, 11, 12, 13
                ...
                if (zero_covid) {
                    ...
                    int model_type;
                    auto it = std::find(stateIndices_PDE.begin(), stateIndices_PDE.end(), homeStateIdx);
                    int stateIdxSpecificModelType;
                    if (it != stateIndices_PDE.end()) { // nr is a PDE nr
                        model_type = pde_idx;
                        stateIdxSpecificModelType = std::distance(stateIndices_PDE.begin(), it);
                    }
                    else {
                        it = std::find(stateIndices_ODE.begin(), stateIndices_ODE.end(), homeStateIdx);
                        if (it != stateIndices_ODE.end()) { // nr is a ODE nr
                            model_type = ode_idx;
                            stateIdxSpecificModelType = std::distance(stateIndices_ODE.begin(), it);
                        }
                        else { // in ABM is true
                            model_type = abm_idx;
                            it = std::find(stateIndices_ABM.begin(), stateIndices_ABM.end(), homeStateIdx);
                            stateIdxSpecificModelType = std::distance(stateIndices_ABM.begin(), it);
                        }
                    }
                    ...
                    if (not greenZone[bundesland]) { // red zone!
                        probability_notAtHome = -1.0; // always at home!
                    }
                }
    ...
    std::tuple< std::vector<std::unordered_map<int, int>>, std::vector<int>, std::vector<std::vector<int>>, std::vector<std::vector<int>>, std::vector<std::vector<int>> > step(const std::vector<std::vector<int>>& event_data, int filtered_event_data_size, std::vector<std::vector<bool>> with_coupling_states, std::unordered_map<int, int>& agentID_location_commuting_start, std::unordered_map<int, int>& agentID_location_commuting_end, std::unordered_map<int, int> agentID_healthStatus, int& step, std::vector<double>& infection_rates, int& nr_day, double& population_scale, std::unordered_map<int, std::tuple<int,int,int>>& mystery_case_location, std::vector<std::vector<std::vector<double>>>& symptomatic_mystery_cases, std::ranlux24_base& gen_local, int& verbosity, int run, int correction_step) { 
        ...
        for (int i = 0; i < filtered_event_data_size; i++) {
            ...
            if (event_data[i][1] == 1) { // activity ends
                ...
                if (no_covid) {
                    for (int infector=0; infector<durations_of_interaction.size(); infector++) {
                        if (random_number <= (1 - std::exp(- (infection_rates[ABM_state_nr] * ci * durations_of_interaction[infector] * sh * in)))) { // change happens
                            ...
                            if (!is_aware_of_being_infected[infector]) { // mystery case!
                                ...
                                int model_type, stateIdxSpecificModelType;
                                auto it = std::find(stateIndices_PDE.begin(), stateIndices_PDE.end(), homeStateIdx);
                                if (it != stateIndices_PDE.end()) { // nr is a PDE nr
                                    model_type = pde_idx;
                                    stateIdxSpecificModelType = std::distance(stateIndices_PDE.begin(), it);
            ...
            if (healthStatus_i == 1) { // exposed
                ...
                if (random_number <= prob_infectious_not_symp_changing_state) { // change happens
                    ...
                    if (random_number < gamma/(phi_i+gamma)) {
                        ...
                        mystery_case_location.erase(agent_id);
                    }
                }
            }
        ...
        return std::make_tuple(healthStatus_agents_in_ABM, agent_IDs_in_ABM, healthStatus_agents_in_PDE, healthStatus_agents_in_ODE, cnt_new_symptomatic);
    }
};