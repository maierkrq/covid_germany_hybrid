 ...
 * 
 * The test does not work with the following preconditioners: PrecondType::NONE, PrecondType::ARMS,  PrecondType::HB. 
...
struct StatHeatTransferSolution
{
   ...
   {
     auto x = cell.geometry().global(localCoordinate);
...
int main(int argc, char *argv[])
{
  ...
  int  maxRefSteps[orderLength] = {4,4,1,1,1};
  //error values for corresponding ansatz order and refinement
  double errorsI[2][5] = {{9.0e-3, 6.5e-3, 2.1e-3, 6.5e-4, 2.1e-4}, //order 1
                          {8.0e-3, 1.7e-3, 2.0e-4, 2.1e-5, 2.4e-6}}; //order 2
  double errorsII[3] = {3.0e-4, //order3
                        2.2e-5, //order4
                        2.1e-6}; //order5
  bool valid = true;
  ...
  property = MatrixProperties::GENERAL;
  ...
  for (int i=0; i<orderLength && valid; i++)
  {
    ...
    std::unique_ptr<Grid> grid( createHexahedron<Grid>() );
    ...
    for (int refSteps=0; refSteps<maxRefSteps[i]+1; refSteps++)
    {
      ...
      assembler.assemble(linearization(F,u),assembler.EVERYTHING,nthreads,verbosity);
      if(verbosity > 0)
        ...
        switch (precondType)
        {
          ...
          case PrecondType::ILUK:
          {
            if(verbosity > 0) {
              ...
              std::cout << "Note that this preconditioner combined with the BICGSTAB solver needs matrix.property = GENERAL" << std::endl;
            }
            int fill_lev = getParameter(pt, "solver.ILUK.fill_lev", 1);
      ...
      interpolateGloballyWeak<PlainAverage>(component<0>(func),StatHeatTransferSolution());
      u -= func;
      L2Norm l2 ;
      double nrm2 = l2(component<0>(u));
      if(verbosity > 0)
      ...
      if(i<2 && !(nrm2<=errorsI[i][refSteps]))
      {
        ...
        message << "[L2-norm of error is " << nrm2 << " but should be below " << errorsI[i][refSteps] << ".]\n";
      }
      else if(i>=2 && !(nrm2<=errorsII[i-2]))
      {
        ...
        message << "[L2-norm of error is " << nrm2 << " but should be below " << errorsII[i-2] << ".]\n";
     }
    }
  }
  ...
  if(result)
  {
    ...
    outfile << "Test with example stationary heat transfer, 3D. Used " << solverTypeI << " solver " << solverTypeII << ":\n"
            <<  message.str() << std::endl << std::endl;
  }
