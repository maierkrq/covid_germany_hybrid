...
#  plots for multithreading in Kaskade7.2 assembly: stiffness matrix (3D)
getenv(var) = system(sprintf("echo $%s", var))
maxthreads = getenv("maxthreads")
set output "speedup_vs_threads_3d_first".maxthreads."_base2.eps"
set terminal postscript enhanced eps 22 color
set title "3D assembly (stiffness matrix) ( graph: 2*threads(2)/threads(n) ) \n on ".getenv("HOSTNAME")
...
"assembsummary3d_order6.stat" using 1:3 t 'p=6' with linespoints ls 11,\
"assembsummary3d_order5.stat" using 1:3 t 'p=5' with linespoints ls 7,\
"assembsummary3d_order4.stat" using 1:3 t 'p=4' with linespoints ls 19,\
"assembsummary3d_order3.stat" using 1:3 t 'p=3' with linespoints ls 24,\
"assembsummary3d_order2.stat" using 1:3 t 'p=2' with linespoints ls 17,\
"assembsummary3d_order1.stat" using 1:3 t 'p=1' with linespoints ls 1
