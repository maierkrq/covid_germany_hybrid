...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < orderLength && valid; ++i)
  {
    ...
    else
    {
      ...
      {
        std::cout << "error = ";
        std::copy(begin(ress),end(ress),std::ostream_iterator<double>(std::cout,", "));
        std::cout << std::endl;
      }
  ...
  if(result)
  {
    ...
    outfile << "Test with embedded error estimation, einspringende Ecke. Used " << solverTypeI << " solver " << solverTypeII << ":\n"
