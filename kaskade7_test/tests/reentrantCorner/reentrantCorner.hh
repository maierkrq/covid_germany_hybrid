...
class ReentrantCornerFunctional: public FunctionalBase<VariationalFunctional>
{
public:
  using Scalar = RType;
...
///
  ...
  {
  public:
    DomainCache(ReentrantCornerFunctional<RType,AnsatzVars> const&,
    ...
    void evaluateAt(Position const& xi, Evaluators const& evaluators)
    {
      ...
      int const TIdx = result_of::value_at_c<typename AnsatzVars::Variables,
                                             0>::type::spaceIndex;
      auto xGlob = e->geometry().global(xi);
      double x = xGlob[0], y = xGlob[1];

      T = component<0>(data).value(at_c<TIdx>(evaluators));
      dT = component<0>(data).derivative(at_c<TIdx>(evaluators))[0];
      
      // Create right hand side for manufactured solution
      // T = x*(x-1)*y*(y-1)*exp(-100*(x-0.5)^2+(y-0.5)^2)
      ...
      gY  = (2*y-1)*x*(x-1);
      gYY = 2*x*(x-1);
      h   = exp( -p*( (x-0.5)*(x-0.5) + (y-0.5)*(y-0.5) ) );
      hX  = -2*p*(x-0.5)*h;
      hXX = -2*p*h - 2*p*(x-0.5)*hX;
      hY  = -2*p*(y-0.5)*h;
      hYY = -2*p*h - 2*p*(y-0.5)*hY;
...
///
  ...
  public:
    ...
    BoundaryCache(ReentrantCornerFunctional<RType,AnsatzVars> const&,
                  typename AnsatzVars::VariableSet const& vars_,
                  int flags=7):
      data(vars_), e(0)
    {}
...
/// \example reentrantCorner.cpp
/// show the usage of ReentrantCornerFunctional describing a stationary heat transfer problem with peak formed source, 
