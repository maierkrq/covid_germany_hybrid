...
namespace Kaskade
{
   ...
   * \tparam SortedRowIdx a range of row indices (global row, local row) sorted ascendingly by global row
   * \tparam SortedColIdx a range of column indices (global col, local col) sorted ascendingly by global col
   */
  ...
  public:
    ...
    /**
     * \brief A sequence of (global row, local row) indices, sorted ascendingly by global row.
     */
    SortedRowIdx ridx() const { return ridx_; }
    
    /**
     * \brief A sequence of (global col, local col) indices, sorted ascendingly by global col.
     */
     ...
     *
     * This is either \f$ nm \f$ for general local matrices or \f$ n \f$ for
     * diagonal matrices.
     */
    size_t size() const
    {
    ...
    /**
     * \name Entry access and assignment
     * @{
     */
     ...
     * \brief Assignment.
     *
     * The given matrix has to have the same size as the local matrix. If the local
     * matrix is diagonal, only the diagonal of \c Aloc is accessed and copied.
     */
   ...
   * 
   * This realizes a container of dense or diagonal matrices (depending on the \b diagonal template parameter),
   ...
   * \tparam SortedRowIdx a range of row indices (global row, local row) sorted ascendingly by global row
   * \tparam SortedColIdx a range of column indices (global col, local col) sorted ascendingly by global col
   * \tparam IT a particular type that can be used to "store" static information associated to the buffer
   */
  template <class Entry, bool diagonal, class SortedRowIdx, class SortedColIdx, 
  ...
  public:
    ...
    /**
     * \brief The number of local matrices that are stored.
     */
     ...
     * 
     * This is generally not necessary, as the LocalMatrices class scatters automatically as soon as its buffer
