...
namespace Kaskade
{
  ...
  /**
   * \ingroup iterative
   * \brief PCG termination after a given number of iterations
   * \tparam R a floating point type for real numbers
   */
  template <class R>
   ...
   * 
   * For greater robustnes, the actual error estimate used is
   * \f[ [\epsilon_{k-l}]^2 := [\epsilon_k]^2 + \sum_{i=k-l}^{k-1} \gamma_i^2. \f] with
  ...
  public:
    ...
    /**
     * \brief Sets a new value for the maximal number of iterations.
     */
    void setMaxit(int m)
    {
      maxit = m;
...
//                             "L must be sequential!");
    ...
    virtual void apply (X& u, Xstar& b, Dune::InverseOperatorResult& res) 
    {
      ...
      if (verbose>0) 
      {            // printing
        std::cout << "=== Kaskade7 IterateType::PCG\n";
        if (verbose>1)
      ...
      for ( ; true; i++ ) 
      {
        ...
        if (qAq <= 0)
          throw NonpositiveMatrixException("encountered nonpositive energy product " + std::to_string(qAq)
                                           + " vs sigma " + std::to_string(sigma) + " in IterateType::PCG.",__FILE__,__LINE__);
