...
namespace Kaskade
{
  ...
  namespace JacobiPreconditionerDetail
  {
    ...
    struct DiagonalBlock  // TODO: implement move constructor for efficient return by ExtractDiagonal functor
    {
      ...
      // mat is the matrix to extract the diagonal from and invert it
      // weight is the Jacobi step size, i.e. the inverted diagonal is multiplied by w
      ...
      : size(mat.N()), dps(NumaThreadPool::instance().nodes())
      {
        ...
        for (auto ri=mat.begin(); ri!=mat.end(); ++ri)
        {
          ...
          size_t const node = uniformWeightRange(static_cast<size_t>(i),nodes,size);     // which NUMA node
    ...
    {
      // w is the Jacobi step size or damping factor
    ...
    public:
       ...
       * \param w the step size or damping factor. For use as a smoother in
       *          multigrid, choose \f$ w < 1 \f$, e.g., in 1D w=0.5, in 2D w=0.67,
       *          and in 3D w=0.7.
       */
   ...
   * \brief A simple single-level Jacobi preconditioner.
   * \tparam Operator satisfying the Dune::AssembledLinearOperator concept. Specializations exist
   *                  for NumaBCRSMatrix and for AssembledGalerkinOperator.
   */
  ...
  /**
   * \ingroup preconditioners
   * \brief DEPRECATED, use JacobiPreconditioner(A) directly.
   * \relates JacobiPreconditioner
   */
  template <class Operator>
  auto makeJacobiPreconditioner(Operator const& op, double w=1.0)
  {
    static Deprecated dummy("makeJacobiPreconditioner() is deprecated, call JacobiPreconditioner(A) directly",2020);
    return JacobiPreconditioner<Operator>(op,w);
  }
  ...
  public:
     ...
     * \brief Default constructor.
     * 
     * Specifying nothing, this leaves the preconditioner in a pretty useless state.
     * Assign a reasonable value before using.
     */
