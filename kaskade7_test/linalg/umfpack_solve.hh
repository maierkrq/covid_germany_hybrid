 ...
 * \tparam Scalar the scalar matrix entry type
 * \tparam SparseIndexInt
 * \tparam DIL the integer type to be used by UMFPack. Either long or int.
 */
...
public:
  ...
  : N(ridx.size())
  , n(n_), Ap(n+1), Ai(N), Az(N), property(property_)
	{
   ...
   */
  virtual SparseIndexInt size() const
  {
    return n;
  }
