...
namespace Kaskade
{
  ...
  UMFSymbolic<double,int,int>::UMFSymbolic(std::vector<int> const & Ap, std::vector<int> const & Ai,
                                           std::vector<double> const & Az, int verbose_) 
  : mem(0), verbose(verbose_)
  {
    if (verbose>=2)
...
{
  assert(xp != bp);
  int status = umfpack_dl_solve(transposed? UMFPACK_At: UMFPACK_A,
                                &Ap[0],&Ai[0],&Az[0],xp,bp,factorization->getMem(),0,0);
  umfpack_dl_report_status(0,status);
}
...
namespace {
  ...
  struct UMFPackCreator64: public Creator<Factorization<double,long> >
  {
    ...
    {
      MatrixAsTriplet<double,long> const& A = std::get<0>(a);
      assert(A.nrows()==A.ncols());

      return std::unique_ptr<Factorization<double,long> >(new UMFFactorization<double,long>(A.nrows(),A.ridx,A.cidx,A.data));
    }
  };
  ...
  struct UMFPackCreator: public Creator<Factorization<double,int> >
  {
    ...
    {
      MatrixAsTriplet<double,int> const& A = std::get<0>(a);
      assert(A.nrows()==A.ncols());

      return std::unique_ptr<Factorization<double,int> >(new UMFFactorization<double,int>(A.nrows(),A.ridx,A.cidx,A.data));
    }
  };
  ...
  struct UMFPackCreator3264: public Creator<Factorization<double,int> >
  {
    ...
    create(Creator<Factorization<double,int> >::Argument a) const
    {
      ...
      return std::unique_ptr<Factorization<double,int> >(new UMFFactorization<double,int,long>(A.nrows(),A.ridx,A.cidx,A.data));
    }
  };
