...
namespace Kaskade
{
  ...
  R PCGEnergyErrorTerminationCriterion<R>::error() const
  {
    ...
    // Estimate local linear contraction rate of the form gamma2(k+i) = a q^i
    // in logarithmic scaling log(gamma2(k+1)) = loga + i logq. Do a least squares fit
    // || [1  0 ]          [log(gamma2(k)  ] ||
    // || [1  1 ] [loga] - [log(gamma2(k+1)] ||
    // || [1  2 ] [logq]   [log(gamma2(k+2)] || = min
    // || [.  . ]          [   ...         ] ||
    // || [1 k-1]          [log(gamma2(k+2)] ||
    //
    // The normal equations are just
    // [m         (m-1)m/2      ] [loga]   [ sum log(gamma2(i))   ]
    // [(m-1)m/2  (m-1)m(2m-1)/6] [logq] = [ sum i*log(gamma2(i)) ]
    int const m = std::min(n,5+lookah);
    Dune::FieldMatrix<R,2,2> AtA(0);
    Dune::FieldVector<R,2> f(0);
    AtA[0][0] = m;           AtA[0][1] = (m-1)*m/2;
    AtA[1][0] = (m-1)*m/2;   AtA[1][1] = (m-1)*m*(2*m-1)/6;
    for (int i=0; i<m; ++i)
    {
      f[0] += std::log(gammas2[n-m+i]);
      f[1] += i*std::log(gammas2[n-m+i]);
    }
    Dune::FieldVector<R,2> x(0);
    AtA.solve(x,f);
    R q = std::exp(x[1]);

    // Estimate error eps^2 = sum gamma^2(i) + q*gamma(n-1)/(1-q)
    R eps2 = std::accumulate(begin(gammas2)+n-lookah,end(gammas2),q*gammas2.back()/(1-q));
    return std::sqrt(eps2);
...
//       epsn2 *= epsn2;
// std::cout << "smoothing pase detected: eps^2=" << epsn2;
//     }
//     else
//     {
//       // second phase: linear convergence
//       epsn2 = pow(theta,2*n)*eps0n2/(1-pow(theta,2*n));
// std::cout << "linear convergence pase detected: eps^2=" << epsn2;
//     }
//
// std::cout << "  total error^2: " << std::accumulate(gammas2.begin()+(n-lookah),gammas2.end(),epsn2) << "\n";
