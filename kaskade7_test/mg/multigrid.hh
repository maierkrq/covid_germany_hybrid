...
namespace Kaskade
{
  ...
  auto makeMultigrid(NumaBCRSMatrix<Entry,Index> const& A, Space const& space, bool onlyLowerTriangle=false, DirectType directType=DirectType::MUMPS)
  {
    ...
    if (order == 1)  // these are (assumed to be) P1 elements
      return std::unique_ptr<Preconditioner>(moveUnique(makeJacobiMultiGrid(A,space.gridManager(),nSmooth,nSmooth,onlyLowerTriangle,directType)));

    if (order == 2)  // P2 elements. For those, a Jacobi smoother is cheaper than and almost as efficient as a patch smoother.
    {
      auto mg = makeJacobiPMultiGrid(A,space,nSmooth,nSmooth,onlyLowerTriangle,directType);
    ...
    // The patch smoother is quite effective, but rather expensive. Hence we use a smaller number of
    // smoothing iterations than for the Jacobi smoother in the h-multigrid part below.
    auto mg = makeBlockJacobiPMultiGrid(A,space,nSmooth,nSmooth,onlyLowerTriangle,directType);
    mg.setSmoothings(3,3);
    return std::unique_ptr<Preconditioner>(moveUnique(std::move(mg)));
  }
