...
namespace Kaskade
{
   ...
   * 
   * Convenience functions for creating additive multigrid preconditioners are available as
   * makeAdditiveMultiGrid and makeBPX.
   * 
   ...
   * \param makeSmoother a callable object taking a (projected) Galerkin matrix of type NumaBCRSMatrix<Entry,Index> and a level,
   *                     returning a preconditioner
   * \param makeCoarseSmoother a callable object taking a (projected) Galerkin matrix of type NumaBCRSMatrix<Entry,Index>
   *                           and a level, returning a preconditioner
   * \param coarseLevel the grid level down to which the additive h-multigrid shall reach
   * \param onlyLowerTriangle if true, A is assumed to be symmetric and only its lower triangular part is accessed
   * 
  ...
  /**
   * \ingroup multigrid
   * \brief Convenience function creating additive multigrid preconditioner of V-cycle type with
   *        domain decomposition and Jacobi smoother for higher order elements.
   *
   * \tparam StorageTag a tag type for selecting the storage of inverse local matrices in the overlapping Schwarz smoother
   * 
   * \param A the sparse Galerkin matrix for higher order finite elements on the leaf view of the given simplicial grid
   * \param space a higher order finite element space
   * \param p1Space a linear finite element space
   ...
   * \param onlyLowerTriangle if true, A is assumed to be symmetric and only its lower triangular part is accessed
   *
   * The space is decomposed in (i) overlapping patches, (ii) a P1 mesh hierarchy, and (iii) a coarse space.
   * For (i), direct solvers are used, resulting in an overlapping additive Schwarz smoother. For (ii),
   * standard point Jacobi with damping factor 0.6 is used. (iii) is again treated with a direct solver.
   * 
   * \see AdditivePMultiGrid
   */
