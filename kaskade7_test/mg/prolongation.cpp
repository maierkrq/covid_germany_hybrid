...
namespace Kaskade
{
  ...
  namespace ProlongationDetail
  {
    ...
    std::vector<MGProlongation> makeProlongationStack(std::vector<Node> parents, int maxLevel, int minLevel, size_t minNodes)
    {
      ...
      for (int l=maxLevel; l>minLevel && parents.size()>minNodes; --l)
      {
    ...
    makeDeformedProlongationStack(std::vector<std::pair<int,std::vector<std::pair<size_t,double>>>> interpolation,
                                  int maxLevel, int minLevel, size_t minNodes)
    {
      std::vector<NumaBCRSMatrix<Dune::FieldMatrix<double,1,1>>> prolongations;
      
      // Step down through the grid hierarchy
      for (int l=maxLevel; l>minLevel && interpolation.size()>minNodes; --l)
      {
        // Find all nodes on coarser levels than l. Assign consecutive coarse level indices to them.
        std::vector<std::pair<int,std::vector<std::pair<size_t,double>>>> coarseNodes;
        std::vector<size_t> indexInCoarse(interpolation.size(),std::numeric_limits<size_t>::max()); // fill with sentinel
        for (size_t i=0; i<interpolation.size(); ++i)
          if (interpolation[i].first < l)
          {
            indexInCoarse[i] = coarseNodes.size();
            coarseNodes.push_back(interpolation[i]);
          }
          
        std::cerr << "prolongation from " << l-1 << " to " << l << " of size " << interpolation.size() << " x " << coarseNodes.size() << "\n";
        // Map all coarse node indices to the coarse grid numbering scheme
        for (auto& n: interpolation)
          for (auto& cn: n.second)
            cn.first = indexInCoarse[cn.first];
            
          
        // Create prolongation 
        NumaCRSPatternCreator<> creator(interpolation.size(),coarseNodes.size());
        for (size_t i=0; i<interpolation.size(); ++i)
          if (interpolation[i].first==l)                      // fine grind node 
            for (auto const& cn: interpolation[i].second)     // enter coarse grid parent nodes 
              creator.addElement(i,cn.first);                 // (which may appear multiple times in the list)
          else                                                // coarse grid node 
            creator.addElement(i,indexInCoarse[i]);           // just remains the same 
            
        NumaBCRSMatrix<Dune::FieldMatrix<double,1,1>> P(creator);
        
        for (size_t i=0; i<interpolation.size(); ++i)
          if (interpolation[i].first==l)                    // fine grind node 
            for (auto const& cn: interpolation[i].second)   // enter prolongation coefficients
              P[i][cn.first] += cn.second;                  // adding up multiple occurences of the same coarse node 
          else                                              // coarse grid node 
            P[i][indexInCoarse[i]] = 1;                     // just coefficient one
        
        prolongations.insert(begin(prolongations),std::move(P));
        
        // reduce to coarse grid nodes 
        interpolation.erase(std::remove_if(begin(interpolation),end(interpolation),[=](auto const& n) { return n.first==l; }),
                            end(interpolation));
      }
  ...
  namespace
  {
    ...
    MGProlongation getAlgebraicProlongation(NumaBCRSMatrix<Entry,Index> const& A)
    {
      ...
      std::vector<std::pair<double,Index>> tmp;                                 // declare here to prevent reallocations
      for (Index i=0; i<A.N(); ++i)
        if (parents[i].level > 0)
        {
          ...
          std::sort(begin(tmp),end(tmp),FirstGreater());                        // sort by connection strength TODO: sorting is waste, we need only the two largest entries...
...
int main(void)
{
  ...
  auto pStack = prolongationStack(gridman);
