...
namespace Kaskade
{
  ...
  public:
    /**
     * \brief Constructor
     * \param space the finite element space 
     * \param targetCondition the condition of patch blocks targeted by regularization 
  ...
  /**
   * \ingroup multigrid
   * \brief Creates a direct solver for the given matrix. This specialization is for matrices containing floats (single precision) as scalar entries.
   * Since the Kaskade interface for direct solvers only works for doubles, the matrix is copied into a new matrix with double as scalar entries.
   * Since the coarse grid matrix often is much smaller, this hopefully means neglectable overhead.
   */
   ...
   * But for 3D 0.4 seems to be better in elastomechanics with flat geometry.
   *
   * \param A the sparse Galerkin matrix for higher order finite elements on the leaf view of the given simplicial grid
   * \param space a higher order finite element space
   * \param nPre the number of pre-smoothings
