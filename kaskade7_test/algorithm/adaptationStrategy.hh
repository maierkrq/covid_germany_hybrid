...
namespace Kaskade
{
  ...
  namespace Adaptivity
  {
	...
	Scalar bulkErrorSquared = std::accumulate(bulkErrorDistribution.begin(),bulkErrorDistribution.end(),0.0,AdaptationStrategy_Detail::Add());
        ...
        for (auto ci=gridManager.grid().leafGridView().template begin<0>(); ci!=cend; ++ci) // iterate over cells
        {
...
//        N2 *= N2;
        ...
        // do refinement
        auto cend = gridManager.grid().leafGridView().template end<0>();
        size_t counter = 0;
...
//                  if(maxErr < errorDistribution[i].first) maxErr = errorDistribution[i].first;
	...
	/*        std::cout << "second refinement" << std::endl;
        auto cend2 = gridManager.grid().leafGridView().template end<0>();
        for(auto ci=gridManager.grid().leafGridView().template begin<0>(); ci!=cend2; ++ci)
        {
          if(ci->level() > 0)
          {
