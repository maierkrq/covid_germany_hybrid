...
namespace Kaskade 
{
  namespace Elastomechanics
  {
    ...
    : orth(orth_), stiffness(0), du0(0), linear(true)
    {
      for (int i=0; i<dim; ++i)
        for (int j=0; j<dim; ++j)
    ...
    : orth(unitMatrix<Scalar,dim>()), stiffness(0), du0(0), linear(true)
    {
    ...
    Scalar OrthotropicLameNavier<Scalar,dim>::d0() const   
    {
        if(linear){
            ...
            auto eps = pack( GreenLagrangeTensor<Scalar,dim>(du0*orth).d0() );
            auto sigma = stiffness * eps;
    ...
    Dune::FieldVector<Scalar,dim> OrthotropicLameNavier<Scalar,dim>::d1(VariationalArg<Scalar,dim> const& arg) const
    {
        ...
        if(linear){
            ...
            GreenLagrangeTensor<Scalar,dim> E(du0*orth);
            auto epsU = pack(E.d0());
            auto sigma = stiffness*epsU;
    ...
    {
        // TODO: I have doubts that this is the most efficient implementation...
        //(LinearizedGreenLagrangeTensor is defined in elasto.hh
        ...
        if(linear){
            ...
            for (int i=0; i<dim; ++i) {
                ...
                for (int j=0; j<dim; ++j) {
                    ...
                    dw[j] = arg2.derivative[0];
                    auto epsW = pack( E.d1(dw*orth) );                    
                    auto epsVW = pack( E.d2(dv*orth,dw*orth));

                    ret[i][j] = sigmaV*epsW + sigma*epsVW; // remember that epsW contains doubled off-diagonal entries. Hence the scalar product
                }                          // of the Voigt vectors is just the tensor contraction.
            }
        }
        return ret;
