...
namespace Kaskade 
{
  ...
  public:
    /**
     * \brief The type of the diffusion tensor.
     * 
     * Conceptually, this is always a dim x dim matrix. For isotropic problems, it is just a multiple of 
     * the unit matrix. In this case, we use a *scalar* diffusion constant.
     */
    using DiffusionTensor = std::conditional_t<isotropic,
                                               Scalar,
    ...
    ScalarLaplace()
    {
      if constexpr (isotropic)
        ...
        kappa = unitMatrix<Scalar,dim,dim>();
    }
    
    /**
     * \brief Specifies a scalar diffusion constant.
     * \param kappa the diffusion constant (has to be symmetric positive definite)
     */
    ...
    {
      auto a = kappa*du0;
      return arg.derivative*a;
    }
    
    Dune::FieldMatrix<Scalar,1,1> d2(VariationalArg<Scalar,dim> const &arg1, VariationalArg<Scalar,dim> const &arg2) const 
    {
      auto a = kappa*arg1.derivative[0];
      return a*arg2.derivative[0];
    }
    
  private:
    DiffusionTensor kappa;               // diffusion constant
