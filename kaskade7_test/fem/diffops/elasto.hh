...
namespace Kaskade 
{
  namespace Elastomechanics
  {
    ...
    public:
      ...
      /**
       * \brief Constructs elastic modulus by material lookup.
       */
      ElasticModulus(std::string const& name)
      : ElasticModulus(ElasticModulus::material(name))
      {}
      
      /**
       * \brief Define material properties in terms of Young's modulus \f$ E \f$ and Poisson's ratio \f$ \nu \f$.
       *
       * This results in \f[ \mu = \frac{E}{2(1+\nu)}, \quad \lambda = \frac{E\nu}{(1+\nu)(1-2\nu)}.\f]
       */
    ...
    {
      // Computes the dimension of a matrix from the length of its Voigt representation vector.
    ...
    public:
       ...
       */
      Tensor d0() const { return static_cast<Scalar>(0.5)*(du+transpose(du)); }
       ...
       */
      Tensor d1(Tensor const& dv) const { return static_cast<Scalar>(0.5)*(dv+transpose(dv)); }
    ...
    public:
      ...
      [[gnu::deprecated("create a new LinearizedGreenLagrangeTensor if needed")]]
      void setDisplacementDerivative(Tensor const& du_)
      {
        du = du_;
      }
       ...
       */
      Tensor d0() const { return static_cast<Scalar>(0.5)*(du+transpose(du)+transpose(du)*du); }
       ...
       */
      Tensor d1(Tensor const& dv) const { return static_cast<Scalar>(0.5)*(dv+transpose(dv)+transpose(dv)*du + transpose(du)*dv); }
       ...
       */
      Tensor d2(Tensor const& dv, Tensor const& dw) const { return static_cast<Scalar>(0.5)*(transpose(dv)*dw + transpose(dw)*dv); }
    ...
    /**
     * \ingroup stationaryElasticity
     * \brief Full (right) Green-Lagrange strain tensor for displacements from dim-1 to dim.
     *
     * 
     * \tparam Scalar the scalar field type, usually double
     * \tparam dim the spatial dimension of the grid coordinates, usually 2
     * \tparam byValue if true (default), the displacement derivative \f$ u_x \f$ is stored by value, otherwise by reference
     */
    template <class Scalar, int dim, bool byValue=true>
    ...
    {
    public:
      typedef Dune::FieldMatrix<Scalar,dim+1,dim> InputTensor;
      typedef Dune::FieldMatrix<Scalar,dim,dim> OutputTensor;
      
      /**
       * \brief Constructor.
       * \param du The spatial derivative of the displacement.
       */
      SurfaceGreenLagrangeTensor(InputTensor const& du_): du(du_), dI(0) 
      {
        for(int i=0; i < dim; ++i)
          dI[i][i] = 1;
//         std::cout << "dI=" << dI << "\n";
//         std::cout << "transpose(dI)=" << transpose(dI) << "\n";
      }
       ...
       * This initializes the displacement derivative to 0, i.e. to the reference configuration.
       */
      SurfaceGreenLagrangeTensor(): du(0), dI(0) 
      {
        for(int i=0; i < dim; ++i)
          dI[i][i] = 1;
      }
       ...
       * \brief The linearized Green-Lagrange strain tensor \f$ E = \frac{1}{2} (u_x + u_x^T + u_x^T u_x) \f$.
       */
      OutputTensor d0() const { return static_cast<Scalar>(0.5)*(transpose(dI)*du+transpose(du)*dI+transpose(du)*du); }
       ...
       * This is \f$ \E_{u_x}(u_x) v_x = \frac{1}{2}(v_x + v_x^T + v_x^T u_x + u_x^T v_x). \f$
       */
      OutputTensor d1(InputTensor const& dv) const { return static_cast<Scalar>(0.5)*(transpose(dI)*dv+transpose(dv)*dI+transpose(dv)*du + transpose(du)*dv); }
       ...
       * This is \f$ E_{u_x,u_x}(u_x) [v_x,w_x] = \frac{1}{2}(v_x^T w_x + w_x^T v_x). \f$
       */
      OutputTensor d2(InputTensor const& dv, InputTensor const& dw) const { return static_cast<Scalar>(0.5)*(transpose(dv)*dw + transpose(dw)*dv); }
      
    private:
      std::conditional_t<byValue,InputTensor,InputTensor const&> du;
      InputTensor dI;
    };
    ...
    public:
      typedef Dune::FieldMatrix<Scalar,dim+1,dim> InputTensor;
      typedef Dune::FieldMatrix<Scalar,dim+1,dim+1> OutputTensor;
      
      /**
       * \brief Constructor.
       * \param du The spatial derivative of the displacement.
       */
      ExtendedGreenLagrangeTensor(InputTensor const& du_): du(du_), duext(extend(du_))
      {
      }
       ...
       * This initializes the displacement derivative to 0, i.e. to the reference configuration.
       */
      ExtendedGreenLagrangeTensor(): du(0), duext(0) 
      {        
      }
      ...
      {
        return static_cast<Scalar>(0.5)*(duext+transpose(duext)+transpose(duext)*duext);         
      }
      ...
      {
        OutputTensor dvext = extend(dv);
        return static_cast<Scalar>(0.5)*(dvext+transpose(dvext)+transpose(dvext)*duext + transpose(duext)*dvext); 
      }
      ...
      {
        OutputTensor dvext = extend(dv);
        OutputTensor dwext = extend(dw); 
        return static_cast<Scalar>(0.5)*(transpose(dvext)*dwext + transpose(dwext)*dvext);         
      }
      
    private:
      
      OutputTensor extend(InputTensor const& d) const
      {
        OutputTensor dext(0);
        ...
        dext[0][dim] = -dext[dim][0];
        dext[1][dim] = -dext[dim][1];
        return dext;
      }
    ...
    public:
       ...
       */
      Tensor d0() const { 
        return E + z.d0()*(E + static_cast<Scalar>(0.5)*unitMatrix<Scalar,dim>());
      }
      ...
      { 
        return dE + z.d1(det.d1(2*dE))*(E + static_cast<Scalar>(0.5)*unitMatrix<Scalar,dim>()) + z.d0()*dE;
      }
      ...
      { 
        return z.d2(det.d1(2*dE1,2*dE2))*(E + static_cast<Scalar>(0.5)*unitMatrix<Scalar,dim>()) + z.d1(det.d1(2*dE1))*dE2 + z.d1(det.d1(2*dE2))*dE1;
      }
    ...
    /**
     * \ingroup stationaryElasticity
     * \brief Computes the dim principal directions of the symmetric positive definite tensor A.
     * 
