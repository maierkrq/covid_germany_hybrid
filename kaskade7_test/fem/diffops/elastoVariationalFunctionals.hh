...
namespace Kaskade 
{
  namespace Elastomechanics
  {
     ...
     * The interface is intended to be used for defining the domain cache of variational functionals.
     * 
    ...
    public:
      ...
      /**
       * \brief Constructor.
       * \param energy a hyperelastic energy functional, will be copied to an internal energy member variable
       * \param strain a strain tensor functional, will be copied to an internal strain variable
       */
      ...
      /**
       * @brief getHyperelasticEnergy provides read access to the stored energy density.
       */
      ...
      /**
       * @brief getDu provides read access to the current displacement derivative.
       */
      ...
      /**
       * @brief getStrain provides read access to the current strain tensor.
       */
      ...
      /**
       * \brief Returns the 2nd Piola-Kirchhoff stress tensor \f$ S = \frac{\partial W}{\partial E} \f$ corresponding to
       *        the current displacement derivative.
       *
       * This is the stress in the reference (material) coordinates, and does *not* provide the actual force density
       * in the deformed configuration. See cauchyStress.
       */
       ...
       *
       * Here, \f$ S \f$ is the second Piola-Kirchhoff stress tensor and \f$ F = I + u_x \f$ the deformation derivative.
       *
       * The Cauchy stress \f$ \sigma \f$, also known as "true" stress, describes the tractions in the deformed coordinates,
       * and therefore the "true" force density on a (cutting) surface.
       */
     ...
     * 
     * This class defines the variational formulation of the Lame-Navier operator
     * \f$ u \mapsto -2\mu\Delta u - \lambda \nabla \mbox{div} u\f$
    ...
    public:
      ...
      /**
       * \brief d1 As the base method, computes the directional derivative of the hyperelastic stored energy density.
       *
      ...
      /**
       * \brief d2 As the base method, computes the second directional derivative of the hyperelastic stored energy density.
       * But this transformed version (only valid in the completely linear case) is more efficient.
       */
    ...
    /**
     * \ingroup stationaryElasticity
     * \brief Represents the first Piola Kirchhoff stress as known in nonlinear elasticity.
     * The Piola-Kirchhoff stress is \f$ T = F\sigma = (I+u_x) \sigma(u) \f$.
     *
    ...
    public:
      ...
      /**
       * @brief FirstPiolaKirchhoffStress constructs first Piola Kirchhoff stress object from current state of elastic energy.
       * @param ef is the elastic energy in current state (and at fixed displacement gradient)
       */
      FirstPiolaKirchhoffStress(EnergyFunctional const& ef) : energy(ef) {}

      /**
       * @brief DEPRECATED, use deformationDerivative instead
       */
      ...
      /**
       * @brief deformationGradient provides access to the current deformation derivative \f$ F = I + u_x \f$.
       */
      ...
      /**
       * @brief d0 provides access to the current first Piola Kirchhoff stress.
       */
      Tensor d0() const
      {
      ...
      /**
       * @brief d1 provides access to the first directional derivative (in direction of some displacement derivative) of the Piola Kirchhoff stress.
       * @param arg is direction of differentiation.
       */
      ...
      /**
       * @brief d2 provides access to the second directional derivative (in directions of some displacement derivatives) of the Piola Kirchhoff stress.
       * Warning: The current implementation is only correct for quadratic stored energy functions, i.e. linear materials. Todo: Fix this.
       *
       * @param argv is the first direction of differentiation.
       * @param argv is the second direction of differentiation.
       */
