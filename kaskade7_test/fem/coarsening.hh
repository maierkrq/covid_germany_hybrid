...
//        p.q = 0 // causes compilation error with dune-2.4.0 and clang++ on OS X (Darwin)
                  ...
                  std::vector<bool>& coarsenedCells, int verbosity=1, int minRefLevel=0)
  {
    ...
    typedef typename Grid::template Codim<0>::Entity CellPointer;
    ...
    CellIterator end = gridManager.grid().leafGridView().template end<0>();
    for (CellIterator ci=gridManager.grid().leafGridView().template begin<0>(); ci!=end; ++ci) // TODO: parallelize this loop
...
//     for (CellIterator ci=gridManager.grid().leafGridView().template begin<0>(); ci!=gridManager.grid().leafGridView().template end<0>(); ++ci)
    for (Cell const& c: Dune::elements(gridManager.grid().leafGridView()))
    {
      auto idx = indexSet.index(c);
      if (c.level()>minRefLevel && std::binary_search(selectedCoarseningGroups.begin(),selectedCoarseningGroups.end(),
                                                        coarseningGroup[idx])) // for uniform refinement: just delete level
      {
        coarsenedCells[idx] = true ; // for compression: coarsening history
        gridManager.mark(-1,c);
      }
    }
