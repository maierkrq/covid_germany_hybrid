...
namespace Kaskade
{
  ...
  /// DEPRECATED: use range based for loops and Dune::intersections() directly.iterates over each face and applies functor to face. Each boundary face is visited exactly once
...
//  }
  ...
  /**
   * \ingroup grid
   * \brief  iterates over each boundary face and applies `functor` to face. Each boundary face is visited exactly once.
   * \param functor a callable object taking a single intersection as argument
   */
  template <class GridView, class Functor>
  void forEachBoundaryFace(GridView const& gridView, Functor functor)
  {
  ...
  {
    forEachCell(gridManager.grid().leafGridView(), [&](typename GridManager::Grid::LeafGridView::template Codim<0>::Entity const& cell)
    {
#ifdef TESTOUTPUT
      ...
      std::for_each(gridManager.grid().leafGridView().ibegin(cell), gridManager.grid().leafGridView().iend(cell), [&](typename GridManager::Grid::LeafGridView::Intersection const& face)
      {
