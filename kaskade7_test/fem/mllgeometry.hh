...
namespace Kaskade
{
  ...
  public:
     ...
     * negligible margin). Defaults to \f$ 100 \epsilon \f$.
     * 
     * \todo: (i) Simplify code by using entity pointer's comparison
    ...
    /**
     * \brief Transformation from global to local.
     *
     * Only those points are returned that lie inside the local
     * entity. All others are deleted during the process.
     *
     * \param x a sequence of global positions. On return, only those
     *          are retained and transformed to local positions that
     *          are inside the local cell.
     * \param componentsInside a sequence of the same length as x. Only the
     *          elements corresponding to retained positions are retained.
     *
     * Usually, componentsInside contains an index list 0,1,...,size(x)-1 on entry.
     * This way, on exit the vector contains the original indices of the retained
     * positions.
     *
