...
namespace Kaskade
{
   ...
   * 
   * Access to the components of the product space is provided by the \ref component function:
   * \code
   * LinearProductSpace<double,boost::fusion::vector<Dune::FieldVector<double,2>,Dune::FieldVector<double,3>>> x;
   * Dune::FieldVector<double,3>& comp1 = component<1>(x);
   * \endcode
   * 
  ...
  public:
     ...
     *
     * For two vectors \f$ (x_i)_{i=0,\dots,n-1} \f$ and \f$ (y_i)_{i=0,\dots,n-1}\f$ this computes and returns
     * the euclidean scalar product
     * \f[ \sum_{i=0}^{n-1} \langle x_i , y_i \rangle. \f]
  ...
  typename boost::fusion::result_of::at_c<Sequence const,m>::type component(LinearProductSpace<Scalar,Sequence> const& x)
  {
    return boost::fusion::at_c<m>(x.data);
  }

  template <int m, class Scalar, class Sequence>
  typename boost::fusion::result_of::at_c<Sequence,m>::type component(LinearProductSpace<Scalar,Sequence>& x)
  {
    return boost::fusion::at_c<m>(x.data);
  }
