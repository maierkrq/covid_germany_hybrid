...
namespace Kaskade
{
  ...
  public:
    ...
    class Combiner
    {
      using GlobalIndices = UniformScalarMapperCombinerArgument<boost::compressed_pair<bool,int>>;

    public:
      Combiner(GlobalIndices globalIndices, ShapeFunctionSet const& /* sfs */)
      : s(globalIndices)
      {}
    ...
    private:
      // Extracts the sign of dof i
      int sign(int i) const { return s[i].second().second(); }

      // A view on the dof range, together with signs for ensuring continuity.
      GlobalIndices s;
    };
...
// std::cerr << "   ContinuousHierarchicExtensionMapperImplementation::dofOnEntity\n";   
        ...
        Dune::GeometryType& gt, int& subentity, int& codim, int& indexInSubentity, Data& data) const
    {
      ...
      gt = this->indexSet().types(codim)[0]; // limited to just one geometry type....
