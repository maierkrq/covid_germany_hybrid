...
public:
  ...
  FullHierarchicIterator( GridImp const& grid_, int maxlevel_, bool end = false ) :
    grid( &grid_ ), coarseGridIt( grid_.levelGridView(0).template begin<0>() ),
    coarseGridEnd( grid_.levelGridView(0).template end<0>() ), hierIt( coarseGridIt->hbegin(maxlevel_) ),
  ...
  // cast to Entity -- needed e.g. for TransferData
  operator typename GridImp::template Codim<0>::Entity ()
        {
    if( coarse || maxlevel==0 ) return typename GridImp::template Codim<0>::Entity( coarseGridIt ) ;
    return typename GridImp::template Codim<0>::Entity( hierIt ) ;
        }
...
public:
  ...
  void update( )
  {
    ...
    for( ElementLevelIterator it = grid.levelGridView( 0 ).template begin<0>();
        it != grid.levelGridView( 0 ).template end<0>(); ++it )
    {
