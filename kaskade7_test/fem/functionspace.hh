...
namespace Kaskade
{
  ...
  /**
   * \ingroup grid
   * \brief Returns a descendant or ancestor cell of the given cell containing the specified local coordinate. 
   * 
   * This method makes use of the grid hierarchy, and is significantly faster than a linear search. 
   *
   * \param cell the start cell
   * \param xi the local position in the start cell for which a descendant cell shall be found
   * \param level grid level on which the target cell is searched (nonnegative)
   * 
   * If level <= cell.level(), we go downwards to coarser levels, until the requested level has been reached.
   * If level > cell.level(), we go upwards until the requested level or the leaf level is reached.
   * 
   * \return a pair (cell,xi)
   * 
  ...
  /**
   * \ingroup grid
   * \brief Returns a cell of the given grid containing the specified global coordinate. 
   * 
  ...
  public:
    ...
    /**
     * \brief type of shape function values 
     */
     ...
     * 
     * Note that even if the type of function (and hence of the function space) is the same,
     * the actual function spaces may be different (e.g., by different polynomial order).
     * If different spaces are involved, assignment is just interpolation.
     */
     ...
     *
     * If the underlying mappers are the same for both FE function, assignment can be done fast by simply copying the coefficients.
    ...
    /**
     * \brief Assignment from a weak function view adaptor.
     *
     * \tparam Functor the actual callable defining the function view
     *
    ...
    /**
     * \brief Assignment from vector of coefficients.
     * 
    ...
    /**
     * \brief Addition of a constant value to the coefficient vector.
     * Note that this does in general *not* add a constant function, except for 
     * the notable case of Lagrange finite elements.
     */
     ...
     * 
     * On codimension >0 entitites, the value is undefined if
     ...
     * 
     * If several values on the same cell are to be calculated and performance is important, consider 
     * creating an evaluator and compute the function's values using the evaluator.
     */
    ...
    /**
     * \brief The polynomial order of the function on the provided cell.
     */
    ...
    //// Order of the shape functions. Appears to be unused and useless. Will be removed.
  ...
  public:
    ...
    struct Evaluator
    {
       ...
       * \param index the index of the cell in the grid view index set. If not provided,
       *              it is looked up (which can be somewhat expensive).
       */
       ...
       * 
       * Note that index lookups are not particularly cheap. Provide the index explicitly
       * for performance reasons if already available.
       */
      ...
      /**
       * \brief Tells the evaluator that subsequent calls to evaluateAt(x,qr,ip,subId) refer to the
       *        given quadrature rule and subentity.
       * 
    ...
    /**
     * \brief Constructs a finite element space from a GridManager, a grid view,
     *        and possibly extra arguments passed to the node manager.
     * 
    ...
    /**
     * \brief Constructs a finite element space from a directly given mapper.
     *
     * This allows constructing a function space without specifying its complete type,
     * which is inferred from the mapper argument (since C++17).
     * \code
     * FEFunctionSpace space(gridman,ContinuousLagrangeMapper<double,GridView>(gridView,order));
     * \endcode
     *
     * \param m the mapper, which is moved into the function space
     */
     ...
     * The returned function is initialized to zero.
     */
    template <int m>
    ...
    /**
     * \brief Returns the coefficients of the linear combination of global DoFs
     *        that define the value at the position specified by the given evaluator.
     *
     * Let \f$ u(x) = \sum_j u_j \varphi_j(x) \f$ and \f$ J(x) := \{ j\in \mathbf{N} \mid
     * x\in \mathrm{supp} \varphi_j \} \f$.
     * Then this method returns \f$ \{ (j,\varphi_j(x)) \mid j \in J(x) \} \f$. This provides
     * information about which global dof affects the value at a certain position, and how much.
     *
     * The same information can be extracted more efficiently but with slightly less
     * convenience directly from the evaluator.
     */
    ...
    /**
     * \brief Returns the coefficients of the linear combination of global DoFs
     *        that define the value at the given position.
     *
     * Let \f$ u(x) = \sum_j u_j \varphi_j(x) \f$ and \f$ J(x) := \{ j\in \mathbf{N} \mid
     * x\in \mathrm{supp} \varphi_j \} \f$.
     * Then this method returns \f$ \{ (j,\varphi_j(x)) \mid j \in J(x) \} \f$. This provides
    ...
    /**
     * \brief Returns the coefficients of the linear combination of global DoFs
     *        that define the value at the given global position.
     *
     * Let \f$ u(x) = \sum_j u_j \varphi_j(x) \f$ and \f$ J(x) := \{ j\in \mathbf{N} \mid
     * x\in \mathrm{supp} \varphi_j \} \f$.
     * Then this method returns \f$ \{ (j,\varphi_j(x)) \mid j \in J(x) \} \f$. This provides
     * information about which global dof affects the value at a certain position, and how much.
     *
     * The same information can be extracted much more efficiently but with less convenience
     * from cell and local position.
     */
    std::vector<std::pair<size_t,SfValueType>>
