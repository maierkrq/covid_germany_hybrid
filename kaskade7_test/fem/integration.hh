...
namespace Kaskade
{
  ...
  /**
   * \ingroup fefunctions
   * \brief Loops over the whole domain occupied by the grid and calls an accumulator
   * for each integration point with the value of the function
   * f. This version of forEach requires the function f to be
   * evaluatable by an Evaluator of the associated space of f.
   *
   * \param f the function to be evaluated
   * \param accumulator a functor with a (possibly non-const) operator()(Cell,
  ...
  /**
   * \ingroup fem
   * @brief integrateOverBoundary computes the integral of an FE function which is restricted to (parts of)
   *        the boundary over the boundary of the underlying grid.
   *
  ...
  /**
   * \ingroup fefunctions
   * \brief Evaluate integrals of finite element functions.
   *
   * This computes \f[ \int_\Omega f(x)\, dx. \f]
   *
   * \tparam a finite element function type, i.e. FunctionSpaceElement<...>, or a FunctionView.
   * \param f a finite element function
   */
