 ...
 * Here, the index \f$ i \f$ of \f$ (\varphi|_T)_i \f$ is local to the cell
 * \f$ T \f$, and thus applies to the restriction \f$ \varphi|_T \f$ and not
 * only to \f$ \varphi \f$.
 * 
 * 
 * Often, \f$ K \f$ is
 ...
 *
 * Combiners \f$ K \in \R^{n\times m} \f$, \f$ m\le n\f$, map the global degrees of freedom to linear
 * combinations of shape functions, which represent the FE ansatz
 * function on the given cell. To be precise, the \f$ m \f$ global ansatz function \f$ \varphi \f$
 * with a nonvanishing restriction \f$ \varphi|_T \f$ to the cell \f$ T \f$ are given as
 * \f[ (\varphi|_T)_i = \sum_{j=1}^n  (\psi\phi_j)K_{ji}  \f]
 * in terms of the \f$ n \f$ local shape functions \f$ \phi_j \f$ and the Converter \f$ \psi \f$. 
 *
 * This implies that if \f$ a\in\R^m \f$ is a finite element coefficient vector (localized to
 * the dofs relevant for cell \f$ T \f$), i.e. \f$ u = \sum_{i=1}^m a_i \varphi_i \f$, then
 * \f$ Ka \in \R^n \f$ is the corresponding coefficient vector for representing \f$ u \f$ in
 * terms of global shape functions, i.e. \f$ u = \sum_{j=1}^n (Ka)_j (\psi \phi_j) = a^T K^T \Psi \Phi\f$.
 *
 * Similarly, if \f$ A \in \R^{k \times n} \f$ contains the values \f$ A_{lj} = \psi\phi_j(\xi_l) \f$
 * of global shape functions \f$ \psi\phi_j \f$ at \f$ k \f$ evaluation points \f$ \xi_l \f$, then
 * \f$ (AK)_{li} = (\varphi|_T)_i (x_l) \f$ contains the values of the ansatz functions at the
 * corresponding points \f$ x_l \f$.
 * 
 * 
...
public:
   ...
   *
   * Let \f$ A\in \R^{k\times n}\f$ with entries \f$ A_{lj} = \psi\phi_j(\xi_l)\f$ being the
   * global shape function values at \f$ k \f$ points \f$ \xi_l \f$.
   * On return, \f$ A \in \R^{k\times m} \f$ contains the corresponding global shape
   * function values, i.e.
   * \f$ A_{li} = (\varphi|_T)_i(x_l) \f$.
   *
   * Similarly, if \f$ A \f$ contains shape function values, on return it contains
   * corresponding ansatz function values.
   *
   * The matrix \f$ A \f$ is resized if needed.
   */
  template <int n, int m>
  void rightTransform(DynamicMatrix<Dune::FieldMatrix<Real,n,m>>& A) const;

  /**
   * \brief In-place computation of row vectors \f$ v \leftarrow v K \f$.
   *
   * This computes ansatz function values from global shape function values
   * as in the matrix case, but just for a single coefficient vector.
   */
  template <int n, int m>
  void rightTransform(std::vector<VariationalArg<Real,n,m>>& v) const;

  /**
   * \brief In-place computation of \f$ A \leftarrow K^+ A \f$. Given a set
...
/**
 * \ingroup concepts
 * \brief A callable that allows to implement weighted (scaled) norms.
 *
 ...
 * trivial \ref IdentityScaling.
 *
 * Note that scalings conforming to this interface can also be implemented
 * as lambda functions.
 */
class Scaling
{
...
/**
 * \ingroup concepts
 * \brief A block filter specifying which subblocks of a stiffness matrix or right hand side shall
 *        be assembled.
 *
 * This maps the row index (rhs, D1) or row and column indices (matrix, D2) to a boolean value.
 * True requests assembly, false leaves the corresponding block untouched.
 */
struct BlockFilter
{
