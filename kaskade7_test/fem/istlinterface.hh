...
namespace Kaskade
{
  ...
  namespace IstlInterfaceDetail {
     ...
     */
    template <class ElementType, class SparseIdx, int rid, int cid, bool sym, bool trans>
    struct Block
    {
      ...
      typedef NumaBCRSMatrix<typename Transpose<BlockType,transposed>::type,SparseIndex> Matrix; // Matrix type in assembler
      ...
      typedef NumaBCRSMatrix<BlockType,SparseIndex> TMatrix;
    ...
    class AllBlocks
    {
      struct Copy
      {
        ...
        auto operator()(MBlock const& mb) const {
          ...
          typedef Block<typename MB::BlockType,typename MB::SparseIndex,MB::rowId,MB::colId,MB::symmetric,false> type;
      ...
      {        
        ...
        struct result<Mirror(MBlock)> {
          ...
          typedef Block<ElementType, typename MB::SparseIndex,
    ...
    struct Translate
    {
      ...
      struct result<Translate(Blk)> {
        ...
        typedef Block<typename Bl::BlockType,typename Bl::SparseIndex,Bl::rowId-firstRow,Bl::colId-firstCol,Bl::symmetric,Bl::transposed> type;
      };

      template <class Bl>
      auto operator()(Bl const& bl) const 
      { 
        Block<typename Bl::BlockType,typename Bl::SparseIndex,Bl::rowId-firstRow,Bl::colId-firstCol,Bl::symmetric,Bl::transposed>  blk(bl.matrix);
    ...
    /**
     * \brief Coordinate mapping
     * \param coefficients an STL sequence type.
     */
    template <class FSElement, class Vector>
    typename std::enable_if<!std::is_same<FSElement,Vector>::value,void>::type
...
*/
  ...
  public:
    ...
    {
//      A.mv(x,b);
            std::vector<Scalar> tmpx, tmpb(A.nrows(),0.0);
    ...
    {
//      A.mv(x,b);
            std::vector<Scalar> tmpx, tmpb(A.ncols(),0.0);
            domainToVector(x,tmpx);
...
//      A.usmv(alpha,x,b);
  ...
  namespace IstlInterfaceDetail
  {
    ...
    struct Base
    {
      typedef typename Assembler::AnsatzVariableSetDescription::template CoefficientVectorRepresentation<firstCol,lastCol>::type Domain;
      typedef typename Assembler::TestVariableSetDescription::template CoefficientVectorRepresentation<firstRow,lastRow>::type   Range;

      static bool const inferredSymmetry
   ...
   *                     The default is to take all blocks in the range given by [firstRow,lastRow[ x [firstCol,lastCol[.
   * \tparam symmetric whether the resulting operator is symmetric. The default is a conservative check 
   *                   whether the operator is symmetric or not.
   *
  ...
  template <class Assembler_,
            int firstRow=0, int lastRow=Assembler_::TestVariableSetDescription::noOfVariables,
            int firstCol=0, int lastCol=Assembler_::AnsatzVariableSetDescription::noOfVariables,
  ...
  public:
    ...
    /**
     * \brief Get coefficient vector \f$coefficients\in\mathbb{K}^m\f$ from \f$y\in Y\f$.
     *
    ...
    /**
     * \brief Get coefficients vector \f$coefficients\in\mathbb{K}^n\f$ from \f$x\in X\f$.
     *
    ...
    /**
     * \name Application of the operator
     * \{
     */
    ...
    /**
     * \brief compute \f$ b \leftarrow A^T x\f$
     */
    void applyTransposed(Range const& x, Domain& b) const
    {
      assert(static_cast<void const*>(&x) != static_cast<void const*>(&b));
    ...
    /**
     * \brief  Compute \f$b \leftarrow b+\alpha Ax\f$
     *
    ...
    /**
     * \brief  Compute \f$b \leftarrow b+\alpha A^T x\f$
     *
     * Note that x and b must not refer to the same memory locations (in case Domain==Range).
     */
  ...
  /**
   * \brief Convenience routine for creating assembled operators.
   * \relates AssembledGalerkinOperator
   */
  template <class Assembler,
            int firstRow=0, int lastRow=Assembler::TestVariableSetDescription::noOfVariables,
            int firstCol=0, int lastCol=Assembler::AnsatzVariableSetDescription::noOfVariables>
  ...
  {
    return AssembledGalerkinOperator<Assembler,firstRow,lastRow,firstCol,lastCol>(assembler);
  }
  ...
  class LagrangeOperator: public Dune::LinearOperator<typename NormalStepAssembler::AnsatzVariableSetDescription::template CoefficientVectorRepresentation<>::type,typename NormalStepAssembler::TestVariableSetDescription::template CoefficientVectorRepresentation<>::type>
  {
    typedef typename NormalStepAssembler::AnsatzVariableSetDescription::template CoefficientVectorRepresentation<>::type Domain;
    typedef typename NormalStepAssembler::TestVariableSetDescription::template CoefficientVectorRepresentation<>::type Range;
    static constexpr int firstRow = 0;
    static constexpr int firstCol = 0;
    static constexpr int lastRow = NormalStepAssembler::TestVariableSetDescription::noOfVariables;
    static constexpr int lastCol = NormalStepAssembler::AnsatzVariableSetDescription::noOfVariables;
