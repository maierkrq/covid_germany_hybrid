...
namespace Kaskade
{
  ...
  public:
    ...
    {
      auto const& simplex = Dune::ReferenceElements<typename Grid::ctype,dim>::simplex();
      update();
    }
    ...
    void update() {
      ...
      for (int i=0; i<sfs.size(); ++i)
        if (std::get<1>(sfs[i].location()) == 0) // codim 0 -> cell function
          ++nCellSf;
        else
          ++nFaceSf;
        
      nCellF = indexSet.size(0) * nCellSf;
      size_t const nFaceF = indexSet.size(1) * nFaceSf;
      nDof = nCellF + nFaceF;
      ...
      for (auto ci=gridView.template begin<0>(); ci!=end; ++ci) 
      {
        ...
        for (int i=0; i<gidx.size(); ++i)
        {
          ...
          else                                    // codim 1 -> face function
          {
            ...
            std::array<int,dim> pi = gbp.barycentricSubsetPermutation(entity);
            std::array<int,dim+1> bc = barycentric(SimplexLagrangeDetail::tupleIndex(order,sf[i].location()),order);
            std::array<int,dim> subBc;
            for (int i=0; i<dim; ++i)
              subBc[i] = bc[pi[i]];
  ...
  public:
    ...
    {
      Dune::ReferenceElement<typename Grid::ctype,dim> const &simplex = Dune::ReferenceElements<typename Grid::ctype,dim>::simplex();
