...
namespace Kaskade
{
  ...
  /**
   * \ingroup fetransfer
   * \brief Computes the values of global shape functions at the
   * given points (which are supposed to be local coordinates inside the
   * cell to which the evaluator is currently moved).
   *
   * This realizes the matrix product \f$\Psi\Phi \f$ (\f$ \Phi \f$ is the
   * matrix of shape function values at the given points \f$ \xi \f$ in the
   * reference element and \f$\Psi\f$ is the converter, see
   * LocalToGlobalMapperConcept). On return, \arg sfValues[i][j]
   * contains the value of global shape function j at global(xi[i]).
   *
   ...
   * (\f$ \Psi \f$ is the converter, and \f$ \Phi \f$ the values of the shape functions at
   * the interpolation nodes, see LocalToGlobalMapperConcept).
   *
   ...
   * 
   * \param[in] globalValues global function values \f$ f(\xi_i) \f$ at interpolation nodes \f$ \xi_i \f$
   *                         of the shape function set (will be overwritten and invalidated)
   * \param[out] coeff coefficients \f$ C \f$ of shape function linear combination such that the global values
   *                   at the interpolation points are approximated: \f$ f(\xi_i) = \sum_j C_{ij} \phi_j(\xi_i) \f$
   * \param sfs the space's shape function set on the given cell (note that the cell needs not be
   *            contained in the space's grid view at all, e.g. when creating multilevel prolongations)
   */
  template <class Space, class ShapeFunctionSet>
  ...
  /**
   * \brief Computes a local prolongation matrix for global shape functions from father cell to child cell.
   * 
   * \return In the notation of LocalToGlobalMapperConcept (\f$\Phi_{ij}\f$ is the matrix of shape function values
   * \f$\phi_j\f$ at interpolation nodes \f$ \xi_i \f$, and \f$\Psi \f$ is the diagonal matrix of converter
   * values \f$ \psi(x_i)\f$), the returned value is
   * \f[ p \leftarrow \Phi_c^+ \Psi_c^{-1} \Psi_f \Phi_f. \f]
   * This means, \f$ p \in \R^{n_c \times n_f} \f$ is a matrix, where the entries \f$ p_{ij} \f$
   * give the contribution of the \f$ j \f$-th father global shape function to the \f$ i \f$-th child
   * global shape function. \f$ n_f \f$ and \f$ n_c \f$ are the number of father and child shape
   ...
   * 
   * \param childSfs a shape function set to use for the child space on the given child cell
   *                 (which need not be contained in the child space's grid view)
   * \param fatherSfs a shape function set to use for the father space on the given father cell
   *                  (which need not be contained in the fathers space's grid view)
   */
  ...
  /**
   * \ingroup fetransfer
   * \brief policy class for LocalTransfer
   * These two structures are used for determining whether to perform a
   * "normal" grid transfer (default, AdaptionCoarseningPolicy),
   * or to calculate prolongation matrices for
   * a FE function space based on a HierarchicIndexSet (see mgtools.hh).
   * \relates MultilevelCoarseningPolicy
   * \relates LocalTransfer
   */
  struct AdaptationCoarseningPolicy
  {
    void update( int ) { }
  ...
  /**
   * \ingroup fetransfer
   * \brief Class that stores for a cell ("this entity" or "father") a local projection matrix.
   *
   * That is, in LocalToGlobalMapperConcept terminology, the matrix
   * \f[ R = \Phi^+ \Psi^{-1} D^{-1} \sum_{\textrm{leaf children } c} \Psi_c \Phi_c K_c. \f]
   ...
   * 
   * \tparam ChildSpace  the projection source, a FEFunctionSpace defined on a (typically the leaf) grid view
   * \tparam FatherSpace the projection target, a FEFunctionSpace
  ...
  private:
    using FatherShapeFunctionSet = typename FatherSpace::Mapper::ShapeFunctionSetImplementation;
...
//     , prolongationMatrix(std::forward<LocalTransferMatrix>(other.prolongationMatrix))
    , restrictionMatrix(std::forward<LocalTransferMatrix>(other.restrictionMatrix))
    , shapeFunctionSet(std::move(other.shapeFunctionSet))
    {    }
    ...
    : globalIndices(other.globalIndices)
//     , prolongationMatrix(other.prolongationMatrix)
    , restrictionMatrix(other.restrictionMatrix)
    , shapeFunctionSet(new typename FatherSpace::Mapper::ShapeFunctionSet(*other.shapeFunctionSet))
    {    }

    /**
     * \brief constructor
     * \param childSpace a finer FEFunctionSpace to be restricted
     * \param fatherSpace a coarser FEFunctionSpace as the target
     * \param flags computes restriction only if flags|RESTRICTION==true, similarly for prolonagtion
     * \param acceptancePolicy
     */
    LocalTransfer(ChildSpace const& childSpace, FatherSpace const& fatherSpace, Cell father, int flags,
                  CoarseningPolicy const& acceptancePolicy = CoarseningPolicy())
    {
      ...
      // Collect all relevant children descending from the father cell. For normal grid transfer, these are
      // the leaf cells.
      ...
      auto const& fatherShapeFunctions = fatherSpace.mapper().shapefunctions(father);

      // Compute the global indices associated to the leaf cells that
      // are descendants from the father cell (possibly including the father itself).
      for (Cell const& child: children)
      {
        auto gi = childSpace.mapper().globalIndices(child);
...
//       prolongationMatrix.setSize(globalIndices.size(),fatherShapeFunctions.size());
//       prolongationMatrix = 0
      ...
      for (Cell const& child: children)
      {
        // Compute the mapping "globIdx" of degrees of freedom on the child to the set of dofs on all children.
        typename ChildSpace::Mapper::GlobalIndexRange gi = childSpace.mapper().globalIndices(child);
        ...
        localProlongationMatrix(childSpace,child,fatherSpace,father,p,
                                childSpace.mapper().shapefunctions(child),
                                childSpace.mapper().shapefunctions(child)); // this makes only sense if the father and child shape functions are the same... TODO: replace this by father - however, as the father cell is not guaranteed to be a leaf cell, there need not be a father shape function set associated with the father cell. Maybe we can extract a "generic" shape function set from the father space?


//         for(size_t k=0; k<globIdx.size(); ++k)
//           for(int j=0; j<p.M(); ++j)
//             prolongationMatrix[globIdx[k]][j] = p[k][j];
        ...
        shapeFunctionSet.reset(new FatherShapeFunctionSet(static_cast<FatherShapeFunctionSet const&>(fatherShapeFunctions)));
        ...
        evaluateGlobalShapeFunctions(childSpace,child,iNodes,afValues);

        // ... and translate them into ansatz function values.
        childSpace.mapper().combiner(child,childSpace.indexSet().index(child)).rightTransform(afValues);
...
//     LocalTransferMatrix prolongationMatrix;
    ...
    std::unique_ptr<FatherShapeFunctionSet> shapeFunctionSet;
  };
  ...
  /**
   * \ingroup fetransfer
   * \brief A class storing data collected before grid adaptation that is
  ...
  public:
    ...
    /**
     * \brief Matrix that transforms a data vector v_1 corresponding to the old
    ...
    public:
      /**
       * \brief Transforms oldCoeff, which lives on the old grid to an equivalent
    ...
    private:
        ...
        tm(rows), nrows(rows), ncols(cols)
      {
        for (int i=0; i<tm.size(); ++i)
        {
          ...
          tm[i].first.clear();  // TODO: this should be redundant - eliminate?
        }
      }
    ...
    /**
     * \brief Gathers all information that might be lost after the refinement
    ...
    : space(space_), DOFcoarseSpace(space.degreesOfFreedom())
    {
      ...
      for (auto cell: elements(space.gridView()))
      {
        ...
        historyData.insert(std::make_pair(idSet.id(cell),RestrictionData(space,space,cell,0,mightBeCoarsened)));
...
// std::cout << "Transfer data gathered: " << historyData.size() << " father cells with restriction data\n";
// for (auto const& p: historyData)
// {
//   std::cout << "id " << p.first << ": child gidx: ";
//   for (auto idx: p.second.globalIndices)
//     std::cout << idx << " ";
//   std::cout << ", restriction matrix:\n" << p.second.restrictionMatrix << "\n";
// }
    }
    ...
    std::unique_ptr<TransferMatrix> transferMatrix() const
    {
      ...
      for (auto cell: elements(space.gridView()))
      {
        ...
        localProlongationMatrix(space,cell,space,ancestor,prolongation,space.mapper().shapefunctions(index),*(restriction->second.shapeFunctionSet));
    ...
    std::unique_ptr<TransferMatrix> transferMatrix(Space2 const& space2) const
    {
      ...
      for (CellIterator ci=space2.gridView().template begin<0,Dune::All_Partition>();
        ...
        RestrictionMatrix prolongation;
        localProlongationMatrix(space,Cell(*ci),space,ancestor,prolongation,restriction.shapeFunctionSet,restriction.shapeFunctionSet);
   ...
   *
   * \tparam Weighing the type of averaging
   * \tparam Target the target function type
   * \tparam Source the source weak function view type
   * 
   ...
   * since this is a little bit faster.
   */
  ...
  /**
   * \ingroup fetransfer
   * \brief An adaptor that turns callables (e.g., lambdas) into weak function views.
   */
  ...
  /**
   * \ingroup fetransfer
   * \brief A convenience function turning callables (e.g., lambda functions) into weak function views.
   *
   * This is a model of the WeakFunctionView concept.
   *
   * \tparam Functor a callable object with operator()(Cell, LocalPosition)
   * \relates WeakFunctionViewAdaptor
   */
  template <class Functor>
   ...
   * \todo implement this as derived class of WeakFunctionViewAdaptor
   */
  ...
  template <class ViewA, class ViewB, class BinOp>
  auto makeWeakFunctionView(BinOp const& op, ViewA const& a, ViewB const& b)
  {
    return makeWeakFunctionView([op,&a,&b](auto const& cell, auto const& xi)
    {
      return op(a.value(cell,xi),b.value(cell,xi));
    });
  }
