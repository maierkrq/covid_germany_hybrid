...
#include "io/readPolyHipJoint.hh"
...
void _readLine(std::istream& in, std::string& line)
{
  do {
    std::getline(in,line);
...
void readNodeData(std::string const& name, std::vector<Dune::FieldVector<double,2> >& nodes, int mortarCheck,
                  std::array<double,3>* pelvis, double scalingFactor)
{
    using namespace std;
    ...
    for (int i=0; i<nVertices; ++i) {
        ...
        nodes[id] *= scalingFactor;

        ////stores obstacle field data (pelvis to be deleted; for testing included)
        //if(obsField!=nullptr){
            ...
            ////Read boundary marker (1 - boundary, 0 - interior)
            //int n;
            //is >> n;
...
std::unique_ptr<Dune::UGGrid<2> > readPolyData(std::string const& name, int heapSize, int mortarCheck, Dune::BitSetVector<1> * implantElement, std::array<double,3>* pelvis, double scalingFactor)
{
  using namespace std;
  ...
  Dune::GridFactory<Grid > factory(grid);
   ...
   // read node file
  std::vector<Dune::FieldVector<double,2> > nodes;
  readNodeData(name+".node",nodes,mortarCheck,pelvis,scalingFactor);
  ...
  if(implantElement != nullptr)
	implantElement->resize(nEle);
  ...
  for (int i=0; i<nEle; ++i) {
    ...
    if(nAttributes != 0 && implantElement != nullptr){
        int n;
        is >> n;
        (*implantElement)[id][0] = n;
    }
