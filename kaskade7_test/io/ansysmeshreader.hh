  ...
  public:
    ...
    std::unique_ptr<Grid> readGrid(std::string const& fileName, int initialGridSize = 1000)
    {
      ...
      if(buffer.find("*HEADING") == NOT_FOUND) 
        throw IOException(std::string("Error reading file ") + fileName + "at line " + std::to_string(currentLine),__FILE__,__LINE__);
      getline(file,buffer);
      ...
      while(file.good())
      {
        ...
        if(buffer[0] != '*') throw IOException(std::string("Expected comment at line " + std::to_string(currentLine) + "."),__FILE__,__LINE__);
                        ...
                        std::vector<std::pair<unsigned int,unsigned int> >& offsets)
      {
        ...
        while(file.good())
        {
          ...
          if(currentIndex+offsets.back().second > givenIndex)
          {
            ...
            throw IOException(std::string("currentIndex > givenIndex in line ") + std::to_string(currentLine),__FILE__,__LINE__);
          }
          // store offsets
          if(currentIndex + offsets.back().second < givenIndex) 
