...
namespace Kaskade
{
  namespace VTKWriterDetail
  {
    ...
    auto vtkTypeNames = boost::fusion::as_map(boost::fusion::make_map<
    ...
    {
      // Note that in VTK, the points located on vertices come first, then the ones on edge midpoints.
      static std::pair<int,int> line[]     = { {0,1}, {1,1}, {0,0} };
      ...
      if (type.isLine())           return line[vtkPoint];
     ...
     * 
     * Since three bytes are encoded as four characters, the last character may depend on the 
     * next data to be written. Therefore a buffer is maintained that stores the data bytes that 
     * could not yet be encoded. On destruction of the writer (or by calling flush()), the buffer 
     * is encoded and the terminal padding characters are written.
     * 
    ...
    public:
       ...
       * \brief Destructor
       * 
       * The destructor calls flush() in order to output any still buffered data and write the 
       * terminal padding. If this IO operation throws an exception, the exception is silently
       * ignored. Should this be an issue, call flush() before destruction and handle the exception 
       * there.
       */
      ~Base64Writer()
      {
    ...
    template<class T>
    VTKDataArrayWriter<T>::VTKDataArrayWriter(std::ostream& s_, IoOptions::OutputType outputType_, 
                                              std::string const& name, int ncomps_,
    ...
    , precision(precision_)
    {
      ...
      s << "<DataArray type=\"" << boost::fusion::at_key<T>(vtkTypeNames) << "\" Name=\"" << name << "\" ";
      ...
      else 
      {
        ...
        //
        // As of Paraview 5.8.1 however, binary .vtu files by Paraview have been encountered missing the header.
    ...
    template void VTKDataArrayWriter<double>::template write<5>(Dune::FieldVector<double,5> const& data);
    template void VTKDataArrayWriter<double>::template write<6>(Dune::FieldVector<double,6> const& data);
    ...
    template void VTKDataArrayWriter<float>::template write<6>(Dune::FieldVector<float,6> const& data);
