  ...
  public:
	...
	*boost::fusion::at_c<0>(x.data) = *(mlTransfer->apply(l, *boost::fusion::at_c<0>(x.data))) ;
		...
		it != itEnd; ++it)
	{
      ...
      for ( VertexLevelIterator it = gridManager.grid().levelGridView(maxLevel).template begin<dim>(); 
	      it != gridManager.grid().levelGridView(maxLevel).template end<dim>(); ++it)
      {
...
//       catch( std::ios_base::failure& e ) { std::cout << e.what() << " Decoding error\n" ; } 
	...
	*previousIndices *= 0 ;  *previousIndices += corr ; 
      ...
      for( int l = coarseLevel ; l <= maxLevel ; l++ )
      {
	VertexLevelIterator itEnd = gridManager.grid().levelGridView( l ).template end<dim>();
	for ( VertexLevelIterator it = gridManager.grid().levelGridView( l ).template begin<dim>(); 
	      it != itEnd; ++it)
	{
	...
	*boost::fusion::at_c<0>(x.data) = *(mlTransfer->apply(l, *boost::fusion::at_c<0>(x.data))) ;
		...
		it != itEnd ; ++it)
	{
      ...
      itEnd = gridManager.grid().levelGridView(maxLevel).template end<dim>();
      for ( VertexLevelIterator it = gridManager.grid().levelGridView(maxLevel).template begin<dim>(); 
	      it != itEnd; ++it)
      {
	(*boost::fusion::at_c<0>(sol.data))[ gridManager.grid().leafIndexSet().index(*it) ] = 
