...
public:
        ...
        number(num), offset(0)
  {
    if (datamode == GnuplotOptions::conforming && git != gend)
      visited[vertexmapper.subIndex(*git,index,n)] = true;
  };
  void increment ()
  {
    switch (datamode)
    ...
    case GnuplotOptions::conforming:
      while(visited[vertexmapper.subIndex(*git,index,n)])
      {
        basicIncrement();
        if (git == gend) return;
      }
  ...
  int id () const
  {
    ...
    case GnuplotOptions::conforming:
      return
      number[vertexmapper.subIndex(*git,renumber(*git,index),n)];
    case GnuplotOptions::nonconforming:
...
protected:
  void basicIncrement ()
  {
    ...
    ++index;
    const int numCorners = git->subEntities(n);
    const int numEdges = git-> subEntities(n-1);
  ...
  int qvmIndex() const{
    if (codim==n)
      return quadvertexmapper.subIndex(*git,subindex,n);
    else //codim == n-1
        return quadvertexmapper.subIndex(*git,subindex,n-1);
  }

  int renumberedqvmIndex() const{
    if (codim==n)
      return quadvertexmapper.subIndex(*git,renumber(*git,subindex,codim),n);
    else
      return quadvertexmapper.subIndex(*git,renumber(*git,subindex,codim),n-1);
  }
...
public:
  ...
  {
    if (datamode == GnuplotOptions::conforming && git != gend)
      visited[quadvertexmapper.subIndex(*git,index,n)] = true;
  };
  void increment ()
  {
    switch (datamode)
...
public:
  ...
  virtual double evaluate (int comp, const Entity& e, const Dune::FieldVector<DT,n>& xi) const
  {
    ...
    for (int i=0; i<e.subEntities(n); ++i)
    {
  ...
  if (order == GnuplotOptions::quadratic) {
    for (CellIterator it=cellBegin(); it!=cellEnd(); ++it) {
      ...
      for (int i=0; i<it->subEntities(n); ++i) {
        ncorners++;
        if (datamode == GnuplotOptions::conforming) {
          int alpha = quadvertexmapper->subIndex(*it,i,n);
          if (number[alpha]<0)
      ...
      for (int i=0; i<it->subEntities(n-1); ++i){
        ncorners++;
        if (datamode == GnuplotOptions::conforming) {
          int alpha = quadvertexmapper->subIndex(*it,i,n-1);
          if (number[alpha]<0)
            number[alpha] = nvertices++;
        }
        else
...
virtual void writeGnuplotData (std::ostream& s, std::ostream& c, std::string name, GnuplotOptions::Info inf)
{
  ...
  for (FunctionIterator it=vertexdata.begin(); it!=vertexdata.end(); ++it) {
    ...
    double m1[ncomps], m2[ncomps], umin[ncomps], umax[ncomps], 
           ptxmin[ncomps], ptymin[ncomps], ptxmax[ncomps], ptymax[ncomps];
